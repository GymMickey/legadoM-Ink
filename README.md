...
<br>
    阅读继承自阅读max，在其基础上删除音频功能包括听书功能、旨为在墨水屏阅读提供良好的阅读环境，并且后期不会考虑增加AI等繁复功能。首先感谢gedoor、Luoyacheng、Horis、Xwite、GEd20、youfengknight、Sum_1的源代码，本项目是在该版本的基础上进行功能的增减。感谢HapeLee，MD3版本的首页设置、目录规则章节数目显示，正是本项目的灵感来源，也是实现该功能的代码参考依据。目前开发者仅会使用ai进行修改，对代码一句不通，期间也经历过跨盘符缓存问题、签名问题导致旧版本无法直接升级新版本等等问题。目前本项目开发者认为已无需要新增功能，开放的目的首先是鼓励各位有志之士，若有兴趣也可以在ai辅助下对项目进行修改。其次目的是若有专业人士感兴趣也可对项目冗余的死代码资源、BUG等部分进行删减，优化等等。
</div>
...
2. **开发者资质**：开发者非计算机专业背景，未学习过 Kotlin、Java 等编程语言及计算机组成原理等基础学科。
...
## 版本说明
- 本项目基于阅读max持续开发，在此基础上删除音频功能、旨为在墨水屏阅读提供良好的阅读环境。
