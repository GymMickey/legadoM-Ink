...
class ExploreShowAdapter(context: Context, val callBack: CallBack) :
    ...
    override fun getViewBinding(parent: ViewGroup): ItemSearchBinding {
        return ItemSearchBinding.inflate(inflater, parent, false).also {
            if (AppConfig.isEInkMode && layoutMode == VIEW_TYPE_LIST) {
                applyEInkCompactLayout(it)
            }
        }
    }

    /** 发现页列表在 E-Ink 下减少无效留白，网格和瀑布流保持原布局。 */
    private fun applyEInkCompactLayout(binding: ItemSearchBinding) = binding.run {
        ...
        // LabelsBar 已是横向单行容器，保持标签可见但不让其换行撑高条目。
        llKind.orientation = android.widget.LinearLayout.HORIZONTAL
    }
